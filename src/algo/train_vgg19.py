...
def train(workers):
    ...
    cbs = [
        callbacks.ModelCheckpoint(config.cfg.TRAIN_PATH.KERAS_MODEL_VGG19, monitor='mean_squared_error', verbose=1,
                                  save_best_only=True, save_weights_only=False, mode='min', period=1),

        callbacks.EarlyStopping(monitor='mean_squared_error', min_delta=0, patience=5, verbose=1, mode='min'),

        callbacks.TensorBoard(log_dir=config.cfg.TRAIN_PATH.TENSORBOARD_LOGS_VGG19, histogram_freq=0,
